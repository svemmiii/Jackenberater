...
- Neue Oberkörper-Grundschicht: JackenBerater unterscheidet jetzt zwischen **Shirt** und **Pullover**, während die bestehende Jackenskala weiterhin die flexible Außenschicht bildet. Hosen/Shorts wurden bewusst nicht in das Modell aufgenommen.
- Pullover wird nicht als späterer Kleidungswechsel geplant. Er gilt für den betrachteten Zeitraum; spätere Änderungen werden weiterhin über abnehmbare Jacken gelöst.
- Bei über mehrere Stunden stabil kühlen Bedingungen kann ein Pullover eine leichte/warme Außenschicht ersetzen oder reduzieren. Bei deutlicher Erwärmung im Tages-/Arbeitsverlauf wird dagegen Shirt + abnehmbare Jacke bevorzugt.
...
- Jackengrenzen-Lernen bei Pullover-Outfits verwendet dieselbe Kleidungswirkung wie die Recommendation. `Perfekt` bei `Pullover + leichte Jacke` bestätigt damit die tatsächlich verwendete Leichtjacken-Grenze statt versehentlich die Warmjacken-Grenze.
- Setup-Speichern ist Single-Flight; ein schneller Doppelklick auf „Profil starten“ sendet nur einen Setup-Request.
- `Perfekt` über einen gesamten Zeitraum zählt die feste Pullover-Grundschicht nur noch einmal als Evidenz; der spätere Kontext darf weiterhin seine eigene Jackengrenze bestätigen, ohne die Pullover-Confidence doppelt zu erhöhen.
- Pullover-Begründungen präzisiert: `flexible_layering` behauptet keine spätere Erwärmung mehr, wenn lediglich Datenlücken, Schwankungen oder Abkühlung vorliegen; `deep_cold` behauptet ohne Forecast keine „anhaltende“ Kälte mehr.
- Pullover-Transientlogik nutzt bei fester Pullover-Grundschicht dieselbe um 3 K verschobene Jackengrenze wie die normale Kleidungsentscheidung; rohe Umgebungs-Effektivtemperaturen bleiben für Diagnose und Lernen unverändert.
- `pullover_reason` ist reine UI-Begründung und gehört nicht mehr zur 10-Minuten-Session-Identität. Eine präzisere/änderte Erklärung ohne geändertes Outfit erzeugt keine zweite Lerngelegenheit.
- Die lernbare Transient-Toleranz kann jetzt bis **0,0** sinken. Wiederholtes eindeutiges Feedback kann damit eine unpassende Kurzzeitglättung vollständig aus der konkreten Entscheidung herauslernen, statt bei 0,5 dauerhaft festzuhängen. Deserialisierung, Feedback-Lernen und Engine verwenden dieselbe Untergrenze.
- `transient_tolerance = 0,0` ist jetzt ein explizites Veto gegen Kurzzeitglättung. Damit kann auch der Grenzfall mit exakt **0 Grad-Minuten** Belastung nicht mehr durch `0 <= 0` als Transient-Override aktiv bleiben.
- Reine Änderungen von `weather.condition` sind keine neue Lernentscheidung mehr. Ein Provider-Wechsel wie `cloudy` → `partlycloudy` bei identischem Outfit-/Lernkontext bleibt innerhalb von zehn Minuten dieselbe Session.
- Reine Änderungen des separaten Regenschutz-Hinweises (`rain_status`, z. B. `none` → `take`) erzeugen ebenfalls keine zweite Wärme-Lernsession mehr, solange Outfit und thermischer Lernkontext unverändert sind. Thermisch wirksamer Regen bleibt über Effektivtemperatur, Jackenklasse und Lernkontext unterscheidbar; dieser Zwischenschritt führte das Session-Schema **v9** ein.
- Reine organisatorische Work-/Kontextmetadaten (`source`, `work_jacket`, `work_context`, `later_work_period`, `later_change_confirmed`) gehören ebenfalls nicht mehr zur trainierbaren 10-Minuten-Session-Identität. Erscheint z. B. ein Arbeitsforecast nachträglich, ohne Outfit oder thermischen Lernkontext zu ändern, bleibt es dieselbe Lerngelegenheit. Tatsächliche Work-bedingte Kleidungsänderungen bleiben über Jackenklasse, `later_at`, Effektivtemperatur und Lernkontext getrennt; dieser Zwischenschritt erhöhte das Session-Schema auf **v10**.
- Session-Reuse trennt jetzt ausdrücklich **gleiche sichtbare Entscheidung** von **gleicher Lernbedeutung**. Kleine Refresh-Abweichungen dürfen weiterhin innerhalb der bestehenden 0,5-/0,25-K-Toleranzen dedupliziert werden, solange sie nicht die von `Perfekt` bestätigte Jackengrenze oder die Aktivierung von Wind-/Transition-Lernen wechseln. Wechselt diese Lernsemantik, wird eine alte unbeantwortete Session superseded und durch einen aktuellen Snapshot ersetzt; beantwortete Sessions mit alter Lernsemantik blockieren keinen aktuellen Lernfall. Das temporäre Session-Schema steht damit auf **v11**.
- Transient-Feedback bei Pullover-Outfits wird jetzt sauber attribuiert: `Pullover + keine Jacke + zu warm` korrigiert die Pullover-Einsatzgrenze statt fälschlich nur die Transient-Toleranz zu erhöhen. `Perfekt` bestätigt den festen Pullover einmal und darf zusätzlich die unabhängige Transient-Entscheidung bestätigen.
- Lernsemantik und Feedback-Policy sind jetzt auf ihre tatsächlich erreichbaren Pfade reduziert: Bei aktivem Transient sind normale Jackengrenzen-, Wind- und Transition-Lerngates für die Session-Identität irrelevant; während Bootstrap sind die Spezialkanäle ebenfalls noch nicht aktiv. Die Feedback-Policy speichert statt roher Confidence nur noch die echte `< 0,55`-Semantik plus Bootstrap-Zustand. Dadurch erzeugen semantisch wirkungslose Refreshes keine neue Opportunity, echte Grenzwechsel bleiben getrennt. Das temporäre Session-Schema steht damit auf **v12**.
- Feedbacksessions besitzen jetzt einen **unveränderlichen Learning Contract**. Beim Öffnen werden Bootstrap/Mature-Modus, das von `Perfekt` bestätigte Jackengrenzen-Ziel, Wind-/Transition-Aktivierung sowie die tatsächlich aktiven Saisongewichte gespeichert; `async_feedback()` setzt genau diesen Vertrag später in `apply_feedback()` durch. Zwischenzeitliches Lernen kann die Bedeutung einer alten Session damit nicht mehr rückwirkend verändern. `PHASE_ALL` bleibt für die komplette Nutzerbewertung in derselben Lernphase. Das temporäre Session-Schema steht damit auf **v13**.
- Die Bootstrap-Grenze wird jetzt über eine gemeinsame Semantik bestimmt und vor der jeweiligen Feedbackinteraktion eingefroren. Ein Stand von z. B. 9,5 Evidenz kann durch die innerhalb derselben Bewertung neu hinzukommende allgemeine Evidenz nicht mehr unvermittelt Wind-/Transition-Speziallernen aktivieren.
- Die Active-Learning-Policy-Signatur speichert im reifen Profil nur noch das tatsächliche OR-Ergebnis `informative_feedback` statt einzelner redundanter Ursachen. Ein Wechsel von `near_threshold` zu niedriger Confidence oder umgekehrt erzeugt bei unverändertem Policy-Ergebnis keine zweite Opportunity. Saisonale Sessionidentität verwendet die tatsächlichen Saisongewichte statt des Kalendertags; ein Mitternachtswechsel innerhalb derselben reinen Saison bleibt dadurch dedupliziert, echte Gewichtsänderungen im Übergangsfenster bleiben getrennt.
- Frontend-Mutation-Flights sind jetzt an Request-Generation, Config Entry und Profil gebunden. Ein hängender Setup-/Maintenance-Request aus einem alten Kontext blockiert denselben Vorgang im neuen Kontext nicht; sein verspätetes `finally` kann einen neueren Lock wegen des separaten Tokens ebenfalls nicht löschen.
- Die 10-Minuten-Deduplizierung bindet Lernevidenz jetzt endgültig an die **reale Entscheidung** statt an deren technische Session-Version: Eine bereits beantwortete current-schema Session bleibt für das volle Fenster Anchor, auch wenn ihr Feedback das Modell und damit einen heute neu berechneten Learning Contract verändert. Dieselbe reale Entscheidung kann dadurch nicht direkt ein zweites Mal gelernt werden.
- Jede trainierbare reale Entscheidung besitzt eine interne `opportunity_count`. Muss ein noch unbeantworteter Snapshot wegen geänderter Lern-/Policy-Semantik ersetzt werden, erbt der neue Snapshot dieselbe Opportunity-Nummer; der globale `feedback_opportunities`-Zähler wird nicht erneut erhöht. Periodische 3er-/10er-Cadence kann dadurch durch Refreshes weder vorgezogen noch übersprungen werden. Das temporäre Session-Schema steht damit auf **v14**.
- Während pausiertem Lernen erzeugte `trainable=false`-Sessions werden innerhalb derselben Pause als Display-Snapshot dedupliziert, statt bei jedem Öffnen eine neue Revision/Store-Änderung zu erzeugen. Nach Resume bleiben sie dauerhaft untrainierbar; eine neue trainierbare Session erhält eine echte neue Opportunity.
